# 打印 man 手册
所有的 man 手册都可以通过 -t 指令输出成 PostScript ，并且通过打印机打印出来。

man -t lp | lp

通过 Google 虽然可以找到关于一个程序的只言片语，但是一般来说最权威，最翔实的
资料，是以程序手册的形式保存下来的。

习惯使用 man 手册去查找某个程序的用法，是一个很好的习惯。

当然你需要有一些耐心，有的程序手册可以长达上万行。
...
DESCRIPTION
       ...
       invocations.   In some cases it is advisable to illustrate several exclusive in‐
...
EXAMPLES
       ...
       man man.7
           Display the manual page for macro package man from section 7.  (This  is  an
           alternative spelling of "man 7 man".)

       man 'man(7)'
           Display  the manual page for macro package man from section 7.  (This is an‐
           other alternative spelling of "man 7 man".  It may be more  convenient  when
           copying  and pasting cross-references to manual pages.  Note that the paren‐
           theses must normally be quoted to protect them from the shell.)
       ...
       man -t bash | lpr -Pps
           Format the manual page for bash into the default troff or groff  format  and
           pipe  it  to  the printer named ps.  The default output for groff is usually
           PostScript.  man --help should advise as to which processor is bound to  the
           -t option.
...
OVERVIEW
       ...
       parse $MANOPT prior to parsing its own command line.  Those options requiring an
       argument  will  be overridden by the same options found on the command line.  To
       reset all of the options set in $MANOPT, -D can be specified as the initial com‐
       mand  line  option.  This will allow man to "forget" about the options specified
       ...
       /usr/share/man.  In some installations, there may also be preformatted cat pages
       to improve performance.  See manpath(5) for details of  where  these  files  are
       stored.

       This package supports manual pages in multiple languages, controlled by your lo‐
       cale.  If your system did not set this up for you automatically,  then  you  may
       need  to  set $LC_MESSAGES, $LANG, or another system-dependent environment vari‐
       able to indicate your preferred locale, usually specified in the POSIX format:
       ...
       of the program, function, or other topic that they document,  and  are  not  in‐
       cluded  with  this package.  If you find that a manual page is missing or inade‐
       quate, please report that to the maintainers of the package in question.
...
DEFAULTS
       ...
       $MANSECT or by the SECTION directive in /etc/man_db.conf.  By default it  is  as
       follows:

              1 1p 8 2 3 3p 3pm 4 5 6 7 9 0p n l p o 1x 2x 3x 4x 5x 6x 7x 8x

       The  formatted manual page is displayed using a pager.  This can be specified in
       a number of ways, or else will fall back to a default (see  option  -P  for  de‐
       tails).

       The  filters are deciphered by a number of means.  Firstly, the command line op‐
       tion -p or the environment variable $MANROFFSEQ is interrogated.  If -p was  not
       used  and  the  environment  variable was not set, the initial line of the nroff
...
OPTIONS
   ...
   Main modes of operation
       ...
       -K, --global-apropos
              ...
              simple  strings  (the default), or regular expressions if the --regex op‐
              tion is used.
       ...
       -W, --where-cat, --location-cat
              ...
              used, then print the locations of all preformatted cat files  that  match
              the search criteria.

              If -w and -W are both used, then print both source file and cat file sep‐
              arated by a space.  If all of -w, -W, and -a are used, then do  this  for
              each possible match.
       ...
       -R encoding, --recode=encoding
              ...
              the source file, you can also use manconv(1) directly.  However, this op‐
              tion  allows  you  to  convert  several manual pages to a single encoding
   ...
   Finding manual pages
       -L locale, --locale=locale
              ...
              sibly including $LC_MESSAGES and $LANG.  To temporarily override the  de‐
              termined  value,  use  this  option to supply a locale string directly to
       ...
       -m system[,...], --systems=system[,...]
              ...
              ual pages, include the system name man in the argument string.  This  op‐
              tion will override the $SYSTEM environment variable.

       -M path, --manpath=path
              Specify  an  alternate  manpath to use.  By default, man uses manpath de‐
              rived code to determine the path to search.  This  option  overrides  the
       ...
       --wildcard
              ...
              pos(1)  --wildcard.   The page argument must match the entire name or de‐
              scription, or match on word boundaries in the description.   Since  there
       ...
       -u, --update
              This  option causes man to update its database caches of installed manual
              pages.  This is only needed in rare situations, and it is normally better
              to run mandb(8) instead.
   ...
   Controlling formatted output
       -P pager, --pager=pager
              Specify which output pager to use.  By default, man  uses  less,  falling
              back to cat if less is not found or is not executable.  This option over‐
              rides the $MANPAGER environment variable, which  in  turn  overrides  the
              $PAGER  environment  variable.   It is not used in conjunction with -f or
              -k.
       ...
       -X[dpi], --gxditview[=dpi]
              ...
              This  option implies -T with the X75, X75-12, X100, or X100-12 device re‐
              spectively.
...
ENVIRONMENT
       ...
       MANROFFOPT
              Every time man invokes the formatter (nroff, troff, or  groff),  it  adds
              the contents of $MANROFFOPT to the formatter's command line.
       ...
       MANSECT
              ...
              it is used to determine which manual sections to search and in  what  or‐
              der.   The default is "1 1p 8 2 3 3p 3pm 4 5 6 7 9 0p n l p o 1x 2x 3x 4x
              5x  6x  7x  8x",  unless  overridden  by   the   SECTION   directive   in
              /etc/man_db.conf.
       ...
       MANPAGER, PAGER
              ...
              By  default, less is used, falling back to cat if less is not found or is
              not executable.
       ...
       MANLESS
              ...
              prompt string”, set $MANLESS to ‘-Psmy prompt string’.  Using the -r  op‐
              tion overrides this environment variable.
       ...
       BROWSER
              ...
              from groff, %% is replaced by a single percent sign (%), and  %c  is  re‐
              placed by a colon (:).
       ...
       MANOPT If $MANOPT is set, it will be parsed prior to man's command line  and  is
              expected to be in a similar format.  As all of the other man specific en‐
              vironment variables can be expressed as command  line  options,  and  are
       ...
       MANWIDTH
              ...
              the value of $COLUMNS, and ioctl(2) if available, or falling back  to  80
...
SEE ALSO
       ...
       lim(1), manpath(5), man(7), catman(8), mandb(8)
